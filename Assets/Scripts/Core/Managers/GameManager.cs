...
namespace UnityRoyale
{
    public class GameManager : MonoBehaviour
    {
        ...
        [Space(20)]
        [Tooltip("List of references of Player's Archery Buildings on Playable Field")]
        [SerializeField] private List<GameObject> playerArcheryBuilding;
        [Tooltip("List of references of Opponent's Archery Buildings on Playable Field")]
        [SerializeField] private List<GameObject> opponentArcheryBuilding;

        [Space(20)]
        public PlaceableData castlePData;
        public PlaceableData archeryBuildingPData;
        ...
        [Space(20)]
        [SerializeField] private List<ThinkingPlaceable> playerUnits, opponentUnits;

        [Space(20)]
        [SerializeField] private List<ThinkingPlaceable> playerBuildings, opponentBuildings;

        [Space(20)]
        private List<ThinkingPlaceable> allPlayers, allOpponents; //contains both Buildings and Units

        [Space(20)]
        [SerializeField] private List<ThinkingPlaceable> allThinkingPlaceables;

        [Space(20)]
        [SerializeField] private List<Projectile> allProjectiles;
        ...
        {
            ActionManager.NavmeshRebuild += ManageNavmeshRebuild;
        }
        ...
        private void Start()
        {
            ...
            playerArcheryBuilding.ForEach(item =>
            {
                SetupPlaceable(item, archeryBuildingPData, Placeable.Faction.Player);
            });

            opponentArcheryBuilding.ForEach(item =>
            {
                SetupPlaceable(item, archeryBuildingPData, Placeable.Faction.Opponent);
            });
        ...
        private void Update()
        {
            ...
            for (int pN = 0; pN < allThinkingPlaceables.Count; pN++)
            {
                ...
                switch (p.state)
                {
                    ...
                    case ThinkingPlaceable.States.Dead:
                        ...
                        break;
                    default:
                        print("No valid state available" + p.state);
                        break;
                }
            }
            ...
            for (int prjN = 0; prjN < allProjectiles.Count; prjN++)
            {
                ...
                if (progressToTarget >= 1f)
                {
                    ...
                    {
                        newHP = currProjectile.target.SufferDamage(currProjectile.damage);
        ...
        private void SetupPlaceable(GameObject go, PlaceableData pDataRef, Placeable.Faction pFaction)
        {
            ...
            switch (pDataRef.pType)
            {
                ...
                case Placeable.PlaceableType.Castle:
                    ...
                    bScript.OnProjectileFired += OnArrowFiredFromTower;
                    ...
                    else if (pDataRef.pType.Equals(Placeable.PlaceableType.Building))
                    {
                        //subscribe building dead method of Archer building
                        bScript.OnDie += OnArcherBuildingDead;
                    }
        ...
        private void OnArrowFiredFromTower(ThinkingPlaceable p)
        {
            Vector3 adjTargetPos = p.target.transform.position;
            Quaternion rot = Quaternion.LookRotation(adjTargetPos - p.projectileSpawnPoint.position);

            FireArrowProjectile fireArrowProj = Instantiate(p.projectilePrefab, new Vector3(p.projectileSpawnPoint.position.x, 4.5f, p.projectileSpawnPoint.position.z), rot).GetComponent<FireArrowProjectile>();
            fireArrowProj.target = p.target;
            fireArrowProj.spawningParent = p;
        }


        private void OnPlaceableDealtDamage(ThinkingPlaceable p)
        {
            if ((p.state != ThinkingPlaceable.States.Dead) && (p.target.state != ThinkingPlaceable.States.Dead))
            {
        ...
        /// <summary>
        /// Method called on Archer building die/collapse
        /// </summary>
        /// <param name="_placeable"></param>
        private void OnArcherBuildingDead(Placeable _placeable)
        {
            _placeable.OnDie -= OnArcherBuildingDead;
        }
        ...
        private void OnPlaceableDead(Placeable p)
        {
            ...
            switch (p.pType)
            {
                ...
                case Placeable.PlaceableType.Castle:
                    ...
                    _ = StartCoroutine(RebuildNavmesh()); //need to fix for normal buildings

                    //we don't dispose of the Castle
                    if (p.pType != Placeable.PlaceableType.Castle)
                        _ = StartCoroutine(Dispose(b));
                    break;

                case Placeable.PlaceableType.Obstacle:
                    _ = StartCoroutine(RebuildNavmesh());
                    break;
